...
// -----------------------------------------------------------------------
// THESIS MODIFICATION: semantic color mapping for smart contract traces.
//
// Perfetto normally colors slices by hashing the slice name, so colors are
// arbitrary. For Ethereum smart contract transaction traces we instead color
// each call by its semantic role, so vulnerability-relevant behaviour is
// visible at a glance:
//   green   = money movement (transfer / withdraw / ETH fallback receive)
//   red     = access control checks (approve / whitelist / owner / controller)
//   orange  = token supply changes (mint / burn)
//   magenta = DEX interaction (swap / exactInput / exactOutput)
//   grey    = read-only view calls (balanceOf / decimals / get* / is* / oracles)
...
const SMART_CONTRACT_COLORS: ReadonlyArray<[RegExp, ColorScheme]> = [
  ...
  [
    ...
    makeColorScheme(new HSLColor({h: 0, s: 80, l: 45})),
  ],
  // Money movement: transfers, withdrawals, ETH fallback receives.
  [
    /transfer|withdraw|fallback/i,
    makeColorScheme(new HSLColor({h: 135, s: 70, l: 40})),
  ],
  // Access control: approvals, whitelist / permission / ownership checks.
  [
    /approve|iswhitelisted|isdisputeperiodover|getcontroller|owner|auth/i,
    makeColorScheme(new HSLColor({h: 0, s: 80, l: 45})),
  ],
  // Token supply changes.
  [/burn|mint/i, makeColorScheme(new HSLColor({h: 28, s: 90, l: 45}))],
  // DEX / swap interaction.
  [
    /swap|exactinput|exactoutput/i,
    makeColorScheme(new HSLColor({h: 300, s: 65, l: 45})),
  ],
  // Read-only view calls.
  [
    /^balanceof|^decimals|^get|^is|latestrounddata/i,
    makeColorScheme(new HSLColor({h: 210, s: 12, l: 62})),
  ],
];
...
export function getColorForSlice(
  ...
  // Smart contract semantics take priority over generic name hashing.
  for (const [pattern, scheme] of SMART_CONTRACT_COLORS) {
